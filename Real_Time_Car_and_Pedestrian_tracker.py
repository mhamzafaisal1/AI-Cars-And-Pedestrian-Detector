...
pedestrian_tracker_classifier_file = 'haarcascade_fullbody.xml'

# create car pedestrian classifier
car_tracker = cv2.CascadeClassifier(classifier_file)
pedestrian_tracker = cv2.CascadeClassifier(pedestrian_tracker_classifier_file)
...
while True:
    ...
    for (x, y, w, h) in cars:
        ...
        # the last two parts are for the color (Blue-Green-Red, thickness of the rectangle)
        cv2.rectangle(frame,  (x, y), (x+w, y+h),
                      (0, 255, 255), 2)  # rand range is a function being used to make sure different colors pop up every time for the face detection
